...
class ReportService:
    ...
    def get_report_by_category(self, user_id, transaction_type=None, start_date=None, end_date=None):
        ...
        report = defaultdict(lambda: {'amount': Money(0), 'type': None})
        for tx in transactions:
            category_name = tx.category.name
            report[category_name]['amount'] = report[category_name]['amount'] + tx.signed_amount
            report[category_name]['type'] = tx.category.type
        report_dict = dict(report)
        sorted_report = dict(sorted(
            report_dict.items(),
            key=lambda x: abs(x[1]['amount'].amount),
            reverse=True
        ))
